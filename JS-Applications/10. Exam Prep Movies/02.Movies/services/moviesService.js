...
async function like(item, id){
    let result = await jsonRequest(`http://localhost:3030/data/likes`, 'Post', item, true);
    return result;
}   
async function userLikes(id, userId){
    let result = await jsonRequest(`http://localhost:3030/data/likes?where=movieId%3D%22${id}%22%20and%20_ownerId%3D%22${userId}%22`);
    return result;
}
